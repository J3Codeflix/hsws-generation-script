...
const inputFolder = "./super-rares";
const outputCharacterJSON = "./outputs/super-rare.json";
...
async function generateCorgis() {
  ...
  while(imgCount < desiredCount) {
    ...
    // upload to IPFS
    const result = await pinata.pinFromFS(`${inputFolder}/approving-corgi${imgCount}${ext}`);
